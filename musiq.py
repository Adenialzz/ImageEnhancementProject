...
class MultiHeadAttention(nn.Module):
    ...
    def forward(self, x, mask=None):
        ...
        k = self.key(x)
        v = self.value(x)
        # logging.info(f"q,k,v shape: {q.shape}")
...
class TransformerBlock(nn.Module):
    ...
    def forward(self, x, inputs_masks):
        ...
        # logger.info(f"inputs_masks shape: {inputs_masks.shape}")
...
class AddHashSpatialPositionEmbs(nn.Module):
    ...
    def __init__(self, spatial_pos_grid_size, dim):
        ...
        logger.info(f"self.position_emb shape: {self.position_emb.shape}")
...
class AddScaleEmbs(nn.Module):
    ...
    def __init__(self, num_scales, dim):
        ...
        logger.info(f"self.scale_emb shape: {self.scale_emb.shape}")
...
class MUSIQ(nn.Module):
    ...
    def forward(self, x, return_mos=True, return_dist=False):
        ...
        x = x[:, :, :-3]
        # logger.info(f"inputs_spatial_postions, {inputs_spatial_positions.shape}")
        # logger.info(f"inputs_scale_positions, {inputs_scale_positions.shape}")
        # logger.info(f"inputs_masks, {inputs_masks.shape}")
